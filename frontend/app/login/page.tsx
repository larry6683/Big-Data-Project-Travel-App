...
import { Mail, Lock, User as UserIcon, ArrowRight, PlaneTakeoff, Loader2 } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (auth.register) await auth.register(name, email, password);
      }
      router.push("/");
    } catch (err: any) {
      ...
      <div className="hidden lg:flex w-1/2 relative bg-slate-900 overflow-hidden items-end justify-start pb-20 pl-16">
        ...
        <div className="relative z-10 max-w-lg text-white animate-in fade-in slide-in-from-left-8 duration-1000 delay-300 fill-mode-both">
          ...
          <h1 className="text-5xl font-black tracking-tight mb-4 leading-[1.1]">
            Your next great adventure, <span className="text-blue-400">planned in seconds.</span>
          </h1>
          <p className="text-lg text-slate-300 font-medium">
            Let our AI craft the perfect itinerary tailored to your unique travel style, budget, and dreams.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center relative p-6 sm:p-12 overflow-hidden">
        ...
        <div className="w-full max-w-[420px] relative z-10">
          ...
          <div className="lg:hidden text-center mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <Link href="/" className="inline-flex items-center gap-2 text-3xl font-extrabold text-slate-900 tracking-tight">
              <PlaneTakeoff className="text-blue-600" size={32} strokeWidth={2.5} />
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] border border-white p-6 sm:p-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="flex bg-slate-100 p-1 rounded-2xl mb-8 relative">
              ...
              <button
                type="button"
                onClick={() => { setIsLogin(true); setGlobalError(""); setFieldErrors({}); }}
              ...
              <button
                type="button"
                onClick={() => { setIsLogin(false); setGlobalError(""); setFieldErrors({}); }}
                className={`flex-1 py-2.5 text-sm font-bold rounded-xl transition-colors duration-300 z-10 ${
                  !isLogin ? "text-slate-900" : "text-slate-500 hover:text-slate-700"
                }`}
              >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              {!isLogin && (
                <div className="animate-in slide-in-from-top-2 fade-in duration-300">
                  <div className="relative group">
                    <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors ${fieldErrors.name ? 'text-red-400' : 'text-slate-400 group-focus-within:text-blue-500'}`}>
                      <UserIcon size={18} />
                    </div>
                    <input
                      type="text"
                  ...
                  {fieldErrors.name && <p className="text-red-500 text-[11px] font-bold mt-1.5 ml-1">{fieldErrors.name}</p>}
                </div>
              )}
              ...
              <div>
                <div className="relative group">
                  <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors ${fieldErrors.email ? 'text-red-400' : 'text-slate-400 group-focus-within:text-blue-500'}`}>
                    <Mail size={18} />
                  </div>
                  <input
              ...
              <div>
                <div className="relative group">
                  <div className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none transition-colors ${fieldErrors.password ? 'text-red-400' : 'text-slate-400 group-focus-within:text-blue-500'}`}>
                    <Lock size={18} />
                  </div>
                  <input
                    type="password"
            ...
            <div className="mt-6">
              <button type="button" className="w-full flex items-center justify-center gap-3 py-3 bg-white border border-slate-200 hover:bg-slate-50 hover:border-slate-300 rounded-xl text-sm font-bold text-slate-700 transition-all shadow-sm active:scale-[0.98]">
          ...
          <p className="text-center text-xs text-slate-500 font-medium mt-8 animate-in fade-in duration-1000">
            ...
            <Link href="#" className="text-blue-600 hover:text-blue-800 transition-colors">Terms of Service</Link> and <Link href="#" className="text-blue-600 hover:text-blue-800 transition-colors">Privacy Policy</Link>.
          </p>
        </div>
      </div>
    </div>
